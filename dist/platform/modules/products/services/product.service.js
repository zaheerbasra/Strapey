...
class ProductService {
    async list(limit = 50) {
        return (0, pg_1.query)('SELECT * FROM products ORDER BY updated_at DESC LIMIT $1', [limit]);
    }
    async getById(productId) {
        const rows = await (0, pg_1.query)('SELECT * FROM products WHERE product_id = $1 LIMIT 1', [productId]);
        return rows[0] || null;
    }
    async create(input) {
        const productId = input.product_id || (0, crypto_1.randomUUID)();
        const rows = await (0, pg_1.query)(`INSERT INTO products(product_id, sku, title, description, brand, category, tags, price, cost, weight, dimensions, inventory, images, videos, attributes, seo_data)
    ...
    async update(productId, input) {
        const rows = await (0, pg_1.query)(`UPDATE products
